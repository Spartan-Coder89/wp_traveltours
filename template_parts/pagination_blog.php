...
<div id="pagination">
    <div class="previous" x-show="max_pages > 1">
        <a href="#" x-on:click.prevent="update_blog_posts(current_page === 1 ? 1 : current_page - 1)">
    ...
    <ul class="pages">
        <template x-for="(item, index) in max_pages" :key="index">
            <li x-bind:class="{'current' : item === current_page}">
                <a href="#" x-text="item" x-on:click.prevent="update_blog_posts(item)"></a>
            </li>
        </template>
    </ul>
    <select id="selection_pages" class="selection_pages" x-on:change="update_blog_posts($event.target.value)">
        <template x-for="(item, index) in max_pages" :key="index">
            <option x-bind:value="item" x-text="item"></option>
        </template>
    </select>
    <div class="next" x-show="max_pages > 1">
        <a href="#" x-on:click.prevent="update_blog_posts((current_page + 1) > max_pages ? max_pages : current_page + 1)">
            <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
